...
export class Dungeon {
    ...
    private _growMaze(start: cc.Vec2) {
        ...
        while (cells !== null && cells.length !== 0) {
            ...
            this.carveGrid(cell.add(this.getDirGridOffset(dir).multiplyScalar(2)), GridType.Floor);
            cells.push(cell.add(this.getDirGridOffset(dir).multiplyScalar(2)));
            lastDir = dir;
        }
    ...
    private isRemove(merged: { [key: number]: number }, connectRegions: { [key: number]: number[] }, connector: cc.Vec2, grid: cc.Vec2) {
        if (connector.subtract(grid).length() < 2) return true;
    ...
    private canCarve(grid: cc.Vec2, dir: DirType) {
        ...
        let nextGrid = grid.add(dirGrid.multiplyScalar(3));
        if (nextGrid.x < 0 || nextGrid.x >= this.width || nextGrid.y < 0 || nextGrid.y >= this.height) {
            return false;
        }

        nextGrid = grid.add(dirGrid.multiplyScalar(2));
